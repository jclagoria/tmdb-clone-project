<script lang="ts">
    ...
    import { ChevronLeft, ChevronRight, Monitor } from "lucide-svelte";
    ...
    <div class="max-w-7xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-2xl font-bold text-white flex items-center gap-2">
                <Monitor class="w-5 h-5 text-tmdb-light" />
                Free To Watch
            </h2>
            <div class="flex gap-4">
                {#each store.tabs as tab}
                    <button
                        ...
                        {store.activeTab === tab.toLowerCase() ? 'tab-active' : 'tab-inactive'}"
                    >
                        {tab}
                    </button>
                {/each}
