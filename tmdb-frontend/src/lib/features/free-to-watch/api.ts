...
import type { FreeToWatchMovie, FreeToWatchResponse, FreeToWatchTV, FreeToWatchTVResponse } from "./types";
...
export async function fetchFreeToWatchTVShows(
    options: {
        language?: string;
        region?: string;
        page?: number;
    } = {}
): Promise<FreeToWatchTV[]> {
    const { language = 'en-US', region = 'US', page = 1 } = options;
    ...
    const response = await fetch(
        `${PUBLIC_CLONE_API_BASE}/free-to-watch/tv?${params.toString()}`
    );
    ...
    const data: FreeToWatchTVResponse = await response.json();
    return data.results;
}

export function transformTVToMediaType(item: FreeToWatchTV): Movie {
    return {
        id: item.id,
        title: item.title,
        original_title: item.originalTitle,
